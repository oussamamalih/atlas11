    ...
    <div class="min-h-screen bg-[#0a1f14] py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
                ...
                <a href="{{ route('scout.search') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded bg-[#10b981] text-[#0a1f14] font-display uppercase text-sm font-semibold hover:bg-[#10b981]/90 transition-colors duration-200">
                        ...
                        <table class="min-w-full divide-y divide-[#1a4030]">
                            ...
                            <tbody class="divide-y divide-[#1a4030]">
                                @foreach($favorites as $favorite)
                                    @php
                                        $playerProfile = $favorite->playerProfile;
                                    ...
                                    <tr class="hover:bg-[#133323]/50 transition-colors duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-3">
                                                <div class="flex-shrink-0 w-10 h-10 rounded-full bg-[#1a4030] overflow-hidden">
                                                    @if($playerProfile?->avatar_url ?? null)
                                                        <img src="{{ $playerProfile->avatar_url }}" alt="{{ $user?->name }}" class="w-full h-full object-cover">
                                                    @else
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($playerProfile?->position ?? null)
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                @if($playerProfile)
                                                    <a href="{{ route('player.profile.show', $playerProfile) }}" class="inline-flex items-center px-3 py-1.5 rounded text-xs font-semibold bg-[#10b981] text-[#0a1f14] font-display uppercase hover:bg-[#10b981]/90 transition-colors duration-200">
                                                        View
                                                    </a>
                                                    <form action="{{ route('favorites.destroy', $playerProfile) }}" method="POST" onsubmit="return confirm('Remove this player from your saved talents?');">
                                                        @csrf
                                                        @method('DELETE')
            ...
            @else
                <div class="rounded border bg-[#0d2919] border-[#1a4030] p-12 text-center">
                    ...
                    <h3 class="font-display text-xl uppercase tracking-wider text-white mb-2">My Saved Talents</h3>
                    <p class="text-[#8fa89c] text-sm mb-6">No saved talents yet. Start exploring to find talent!</p>
                    <a href="{{ route('scout.search') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded bg-[#10b981] text-[#0a1f14] font-display uppercase text-sm font-semibold hover:bg-[#10b981]/90 transition-colors duration-200">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        Search Players
                    </a>
